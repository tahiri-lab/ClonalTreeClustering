...
def analyze_tree_pairs(trees):
    """
    Analyze common and uncommon nodes between pairs of trees and calculate sums of normalized values.
    """
    normalized_weight_matrix, normalized_branch_matrix, seq_list = create_matrices(trees)
    
    # Initialize results dictionaries for weights and branches
    weight_results = []
    branch_results = []
    
    # Analyze all pairs of trees
    for (i, tree1), (j, tree2) in combinations(enumerate(trees), 2):
        ...
        # Calculate sums for branch lengths
...
def format_results(results, title):
    ...
    """
    output = [f"\n{title}:", "Tree Pair | Common Nodes Sum | Uncommon Nodes Sum"]
    output.append("-" * 50)
...
def main():
    ...
    with open(output_file, 'w') as f:
        f.write(format_results(weight_results, "Normalized Weight Differences"))
        f.write(format_results(branch_results, "Normalized Branch Length Differences"))
