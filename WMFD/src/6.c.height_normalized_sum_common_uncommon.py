...
def analyze_tree_differences():
    ...
    for i in range(n_trees):
        for j in range(i + 1, n_trees):
            ...
            all_nodes = tree_nodes[i] | tree_nodes[j]
            ...
            common_sum = (np.sum(diff[common_indices]) / num_common) if num_common > 0 else 0
            uncommon_sum = (np.sum(diff[uncommon_indices]) / num_uncommon) if num_uncommon > 0 else 0
...
def write_results_to_file(results, output_file):
    with open(output_file, 'w') as f:
        f.write("Results Matrix (Normalized by Node Counts):\n")
        f.write("-" * 90 + "\n")
        f.write(f"{'Tree Pair':<15} {'Common Nodes Sum':>20} {'Uncommon Nodes Sum':>20} "
                f"{'#Common Nodes':>15} {'#Uncommon Nodes':>15}\n")
        f.write("-" * 90 + "\n")
