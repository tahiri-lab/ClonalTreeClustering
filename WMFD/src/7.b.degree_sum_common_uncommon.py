...
def extract_nodes_from_tree(tree_str):
    """Extract all valid nodes from a tree string."""
    root = parse_custom_newick(tree_str)
    nodes = set()
    
    def traverse(node):
        if node.name and is_valid_node_name(node.name):
            nodes.add(clean_node_name(node.name))
        for child in node.children:
            traverse(child)
...
def analyze_tree_differences():
    ...
    output_path = os.path.expanduser('~/1.mahsa.farnia/classificataion_journal/degree_comparison_results.txt')
    ...
    for (i, nodes1), (j, nodes2) in combinations(enumerate(tree_nodes), 2):
        ...
        uncommon_nodes = (nodes1 | nodes2) - common_nodes
    ...
    with open(output_path, 'w') as f:
        f.write("Normalized Degree Differences:\n")
        f.write("Tree Pair              Common Nodes Sum  Uncommon Nodes Sum\n")
        f.write("-" * 55 + "\n")
        for result in results:
            f.write(f"{result['trees']:<20} {result['common_sum']:>14.4f} {result['uncommon_sum']:>16.4f}\n")
