...
int approx_arb(double **DISS,double **TM,double **TMnew,double **W, int *Iternum, long int *ARETE, double *LONGUEUR,int choix,int *kt,int binaire,int n)
{
	...
	{    
		cpt++;
		//printf("%d ",cpt);
		if(cpt > 100000) {printf("\nProbleme approx_arb"); return -1;}
		for (i=1; i<=na; i++)//-
		{
			if (Score[EX1[i]]==5) 
				Score[EX1[i]]=3;
						...
						std::map <std::string, int>& namesMap, std::map <std::string, int>& abondMap)
{
	...
	*/
	...
	//Correctness of the Newick format verification done in the main of the n2m_Lineage.cpp script
	i=0;
	n = 0;
	...
	while (string[0] == '(')   // traiter toute la chaine
	{
		...
		if(string[suiv] == '@')
		{
			...
			//printf("\n nom des nodes sans noms %d", ancetre);
